...
class AuditFeatureTest extends TestCase
{
    ...
    {
        // Hit the events endpoint to verify event generation/retrieval
    ...
    {
        // Hit the audits endpoint to verify audit generation/retrieval
    ...
    {
        // Submitting an attempt should log an audit
        $payload = [
            'attempt_uuid' => $this->attempt->uuid
        ];
        $this->actingAs($this->user)->json('POST', '/api/v1/attempts/' . $this->attempt->uuid . '/submit', $payload);
    ...
    {
        // Saving an answer should log an audit
        $payload = [
            'question_uuid' => Str::uuid()->toString(),
            'answer_payload' => '{"selected": ["B"]}'
        ];
        $this->actingAs($this->user)->json('POST', '/api/v1/delivery/attempts/' . $this->attempt->uuid . '/answers', $payload);
