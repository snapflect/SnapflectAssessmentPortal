...
class AssessmentPublicationService
{
    ...
    public function makeReady(string $assessmentUuid, int $organizationId, int $userId): PublicationResultDto
    {
        return DB::transaction(function () use ($assessmentUuid, $organizationId, $userId) {
            ...
            $targetStatus = PublicationStateMachine::STATE_READY;
            ...
            if (!$validationResult->readyForPublication) {
                throw new AssessmentPublicationException(AssessmentPublicationException::VALIDATION_REQUIRED, "Assessment failed validation and cannot be marked as READY.");
            }
    ...
    public function publish(string $assessmentUuid, int $organizationId, int $userId): PublicationResultDto
    {
        return DB::transaction(function () use ($assessmentUuid, $organizationId, $userId) {
            $assessment = $this->assessmentRepository->query()
                ->where('uuid', $assessmentUuid)
            ...
            $validationResult = $this->validationService->validate($assessmentUuid, $organizationId, $userId);
            if (!$validationResult->readyForPublication) {
                throw new AssessmentPublicationException(AssessmentPublicationException::ASSESSMENT_NOT_READY, "Assessment validation failed during publish attempt. Validation Engine is the source of truth.");
            }
