...
@Component({
  ...
  template: `
        ...
        <div *ngSwitchCase="0">
          <app-eof-card 
            ...
            <form [formGroup]="basicForm" class="space-y-6">
              ...
              <div class="pt-2">
                <label class="block text-sm font-semibold text-main mb-2">Workspace Alias (Optional)</label>
    ...
    <app-eof-success
      *ngIf="isSuccess"
      headerTitle="🎉 Workspace Ready"
      description="{{ basicForm.value.organization_name }} has been provisioned successfully. Database, Storage, and Security environments are live."
...
export class OrganizationCreatePageComponent implements OnInit {
  ...
  constructor() {
    this.basicForm = this.fb.group({
      organization_code: ['', [Validators.pattern('^[a-zA-Z0-9_-]+$')]],
  ...
  ngOnInit() {
    this.eofConfig = {
      wizardId: 'org_create_v1',
      entityType: 'organization',
      title: 'Provision Workspace',
      description: 'Configure tenant identity and billing before activation.',
      initialState: 'draft',
      steps: [
        {
          id: 'basic',
  ...
  onComplete(data: any) {
    ...
    this.isLoading = true;
    console.log('Creating organization with payload:', finalData);
    this.http.post('http://localhost:8000/api/v1/governance/organizations', finalData)
      .subscribe({
        next: () => {
