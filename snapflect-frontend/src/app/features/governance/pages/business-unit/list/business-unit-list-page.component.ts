  ...
  template: `
    <div class="h-full flex flex-col relative">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h2 class="text-2xl font-bold text-white">Business Units</h2>
          <p class="text-slate-400 text-sm mt-1">Manage broad organizational groups and cost centers.</p>
        </div>
      ...
      <div class="glass-card flex-1 overflow-hidden flex flex-col">
        <div class="p-4 border-b border-white/10 flex justify-between items-center bg-black/20">
          <div class="relative w-64">
            ...
            <input type="text" [(ngModel)]="searchTerm" class="input-field pl-10 py-2 text-sm bg-surface-darker/50" placeholder="Search business units...">
          </div>
          ...
          <table class="w-full text-left text-sm text-slate-300">
            ...
            <tbody [class.opacity-50]="loading" [class.pointer-events-none]="loading" class="transition-opacity duration-300">
              ...
              <ng-container *ngIf="businessUnits | globalSearch: searchTerm as filteredBUs">
                ...
                <tr *ngFor="let bu of filteredBUs" class="border-b border-white/5 hover:bg-white/5 transition-colors">
                  <td class="px-6 py-4 font-medium text-brand-light">{{ bu.attributes.business_unit_code }}</td>
                  <td class="px-6 py-4 text-white font-medium">{{ bu.attributes.business_unit_name }}</td>
                  <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4 text-right space-x-3">
                    <button class="text-slate-400 hover:text-white transition-colors" (click)="openEditForm(bu)">Edit</button>
                    <button class="text-slate-400 hover:text-red-400 transition-colors" (click)="deleteBU(bu.uuid)">Delete</button>
                  </td>
                </tr>
              </ng-container>
            </tbody>
      ...
      <app-slide-over [isOpen]="isSlideOverOpen" 
        ...
        <form [formGroup]="buForm" (ngSubmit)="submitForm()" class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1">Business Unit Code *</label>
            <input type="text" formControlName="business_unit_code" class="input-field" placeholder="e.g. SALES">
          </div>

          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1">Business Unit Name *</label>
