  ...
  template: `
    <div class="h-full flex flex-col relative">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h2 class="text-2xl font-bold text-white">Assessment Categories</h2>
          <p class="text-slate-400 text-sm mt-1">Manage global categories used to classify assessments.</p>
        </div>
        <div>
      ...
      <div class="glass-card flex-1 overflow-hidden flex flex-col">
        <div class="p-4 border-b border-white/10 bg-black/20 flex justify-between items-center">
          <p class="text-sm text-slate-400">
            Showing <span class="text-white font-medium">{{ categories.length }}</span> categories
          </p>
          <div class="relative w-64">
            ...
            <input type="text" [(ngModel)]="searchTerm" class="input-field pl-10 py-1.5 text-sm bg-surface-darker/50" placeholder="Search categories...">
          </div>
        </div>
          ...
          <table *ngIf="!loading" class="w-full text-left text-sm text-slate-300">
            ...
            <ng-container *ngIf="categories | globalSearch: searchTerm as finalCategories">
              <tbody>
                ...
                <tr *ngFor="let c of finalCategories" class="border-b border-white/5 hover:bg-white/5 transition-colors">
                  <td class="px-6 py-4 font-medium text-brand-light">{{ c.attributes.category_code }}</td>
                  <td class="px-6 py-4">
                    <p class="text-white font-medium">{{ c.attributes.category_name }}</p>
                  ...
                  <td class="px-6 py-4">
                    <span class="px-2 py-0.5 text-[10px] font-semibold rounded uppercase"
                          [ngClass]="c.attributes.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-500/10 text-slate-400 border border-slate-500/20'">
                      {{ c.attributes.status }}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-right space-x-3">
                    <button class="text-slate-400 hover:text-white transition-colors" (click)="openEdit(c)">Edit</button>
                    <button class="text-slate-400 hover:text-red-400 transition-colors" (click)="deleteCategory(c.uuid)">Delete</button>
                  </td>
                </tr>
              </tbody>
            </ng-container>
