  ...
  template: `
    <div class="h-full flex flex-col relative">
      ...
      <div class="glass-card flex-1 overflow-hidden flex flex-col">
        ...
        <div class="overflow-auto flex-1">
          ...
          <table *ngIf="!loading" class="w-full text-left text-sm text-muted">
            ...
            <ng-container *ngIf="types | globalSearch: searchTerm as finalTypes">
              <tbody>
      ...
      <app-slide-over [isOpen]="isFormOpen"
                      ...
                      description="Manage assessment type details."
...
export class TypeListPageComponent implements OnInit {
  ...
  openEdit(t: AssessmentType) {
    this.editingType = t;
    this.form.patchValue({
      type_code: t.attributes.type_code,
      type_name: t.attributes.type_name,
      status: t.attributes.status,
      description: t.attributes.description
    });
    this.form.get('type_code')?.disable();
    this.isFormOpen = true;
  }
  ...
  submit() {
    ...
    const req = this.editingType
      ? this.http.put(`${environment.apiUrl}/assessment/types/${this.editingType.uuid}`, payload)
      : this.http.post(`${environment.apiUrl}/assessment/types`, payload);

    req.subscribe({
      next: () => { 
        this.submitting = false; 
        this.toastService.success('Type Saved', 'The type has been saved successfully.');
        this.closeForm(true); 
        this.fetchTypes(); 
      },
      error: (err) => { 
        this.submitting = false;
        const msg = err.error?.message || 'Failed to save type.';
        this.toastService.error('Error', msg);
      }
    });
  }

  async deleteType(uuid: string) {
    const confirmed = await this.confirmService.confirm({
      title: 'Delete Type',
    ...
    if (confirmed) {
      this.http.delete(`${environment.apiUrl}/assessment/types/${uuid}`)
        .subscribe({ 
          next: () => {
            this.toastService.success('Type Deleted', 'The type was removed successfully.');
            this.fetchTypes();
          } 
        });
    }
