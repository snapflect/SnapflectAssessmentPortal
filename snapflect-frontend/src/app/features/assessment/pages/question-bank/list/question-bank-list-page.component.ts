  ...
  template: `
    <div class="h-full flex flex-col relative">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h2 class="text-2xl font-bold text-white">Question Banks</h2>
          <p class="text-slate-400 text-sm mt-1">Organize and manage your assessment questions into distinct banks.</p>
        </div>
      ...
      <div class="glass-card flex-1 overflow-hidden flex flex-col">
        <div class="p-4 border-b border-white/10 flex justify-between items-center bg-black/20">
          <div class="relative w-64">
            ...
            <input type="text" [(ngModel)]="searchTerm" class="input-field pl-10 py-2 text-sm bg-surface-darker/50" placeholder="Search question banks...">
          </div>
        </div>
          ...
          <table class="w-full text-left text-sm text-slate-300">
            ...
            <tbody [class.opacity-50]="loading" [class.pointer-events-none]="loading" class="transition-opacity duration-300">
              ...
              <ng-container *ngIf="banks | globalSearch: searchTerm as filteredBanks">
                ...
                <tr *ngFor="let bank of filteredBanks" class="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td class="px-6 py-4 text-right space-x-3">
                    <button class="text-slate-400 hover:text-white transition-colors" (click)="openEditForm(bank)">Edit</button>
                    <button class="text-slate-400 hover:text-red-400 transition-colors" (click)="deleteBank(bank.uuid)">Delete</button>
                  </td>
                </tr>
              </ng-container>
            </tbody>
      ...
      <app-slide-over [isOpen]="isSlideOverOpen" 
        ...
        <form [formGroup]="bankForm" (ngSubmit)="submitForm()" class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1">Bank Code *</label>
