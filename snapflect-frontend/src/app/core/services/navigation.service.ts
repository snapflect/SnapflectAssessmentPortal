...
export interface NavLink {
  ...
  path: string;
  icon: string;
  group?: string; // e.g., 'Governance', 'Authoring', 'Delivery'
  permissions?: string[]; // If undefined, available to all
}
...
export class NavigationService {
  ...
  // Master list of all possible links with their required roles
  private allLinks: NavLink[] = [
    ...
    { name: 'Billing', path: '/governance/billing', icon: this.icons.billing, group: 'Governance', permissions: ['Billing.Invoices.View', 'Billing.Subscription.Manage'] },
  ...
  public navGroups = computed<NavGroup[]>(() => {
    ...
    // Group definitions to maintain order
    const groupOrder = ['Home', 'Governance', 'Authoring', 'Delivery', 'Analytics', 'Learner Portal'];
    ...
    // Catch-all for any groups not in groupOrder
    groupsMap.forEach((links, g) => {
      if (!groupOrder.includes(g)) {
        result.push({ name: g, links });
      }
    });
