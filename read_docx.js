...
async function extractTextFromDocx(filePath) {
    return new Promise((resolve, reject) => {
        fs.readFile(filePath, (err, data) => {
            ...
            // Very naive ZIP parsing to find word/document.xml
            // ZIP local file header signature: 0x04034b50
            let offset = 0;
            let documentXmlData = null;
            
            while (offset < data.length - 30) {
                const signature = data.readUInt32LE(offset);
                if (signature !== 0x04034b50) {
                    offset++; // Scan for next signature if not aligned
                    continue;
                }
                ...
                if (fileName === 'word/document.xml') {
                    ...
                    zlib.inflateRaw(compressedData, (err, buffer) => {
                        ...
                        // Extract text from XML <w:t> tags
                        let text = '';
...
async function main() {
    ...
    const files = [
        'Snapflect Assessment Portal - 027 ASSESSMENT SCHEMA DESIGN v1.0.docx',
        'Snapflect Assessment Portal - 027A ASSESSMENT SCHEMA Part 1 v1.0.docx',
        'Snapflect Assessment Portal - 027A ASSESSMENT SCHEMA Part 2 v1.0.docx',
        'Snapflect Assessment Portal - 028 ASSESSMENT OPENAPI v1.0.docx',
        'Snapflect Assessment Portal - 029 SPRINT 02 IMPLEMENTATION PLAN v1.0.docx'
    ];
    ...
    fs.writeFileSync('D:\\Mubarak\\SnapFlectMobileWebApp\\Snapflect Assessment Portal\\extracted_docs.txt', combinedText);
